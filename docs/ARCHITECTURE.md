...
**Status:** Accepted — Milestone 1 (Foundations + Auth & Profiles) implemented & verified 2026-07-19; Milestone 2 (Requests, AI diagnosis, pricing) implemented & verified 2026-07-20; Milestone 3 (Matching & dispatch, PostGIS, real-time offers) implemented & verified 2026-07-21; Milestone 4 (Job execution) and Milestone 5 (Payments — mock provider) implemented & verified 2026-07-21
**Date:** 2026-07-18 (v3.4 — Milestone 4+5 completion folded in 2026-07-21; see roadmap note below on M4/M5 scope vs. this document's original numbering)
...
| **M4 — Job execution** ✅ *(retitled from this doc's original "Money" scope — see note below)* | 1 wk | `ASSIGNED→EN_ROUTE→IN_PROGRESS→COMPLETED→CLOSED` state machine, master en-route/start/complete actions, completion-evidence upload (reuses `OrderMedia`), customer confirmation, `Order.finalAmount` settlement bridge, master job history. | **Done 2026-07-21** |
| **M5 — Payments (mock provider)** ✅ *(a subset of this doc's original "Money" scope — subscriptions/wallet/payouts/reconciliation NOT built, see note)* | 1 wk | `PaymentProvider` abstraction + `MockPaymentProvider` (no real Click/Payme/Uzum credentials), order payments + 1% tax withholding wired to a real flow for the first time, platform-fee settlement, master earnings history, webhook endpoint (documented extension point, not yet exercised). | **Done 2026-07-21** |
...
> **Note on M4/M5 renumbering (2026-07-21):** the founder's actual Batch 1 instruction defined M4 as "Job Execution" and M5 as "Payments & Financial System" — narrower and differently scoped than this table's original M4 ("Money": payments + subscriptions + wallet + payouts + reconciliation) and M5 ("Trust & incentives": trust tiers, penalties, Handly Guarantee, cashback, referrals, loyalty). The rows above now describe what was actually built under the M4/M5 labels. **Not yet built and not yet assigned a milestone number:** subscriptions (Free/Premium/trial/renewal), earnings wallet + payouts + reconciliation, trust-tier recompute + penalty engine, Handly Guarantee claims/rework/refund flows, cashback/credits wallet, referrals, loyalty rates. The founder's Batch 2 was named "M6 + M7" — but this table's existing M6/M7 rows (Admin dashboard; Hardening & beta) don't cover that leftover scope either. **This needs founder clarification before Batch 2 starts:** either fold the leftover Money/Trust-&-incentives scope into new M6/M7 definitions (renumbering Admin dashboard/Hardening further out), or confirm Batch 2's "M6 + M7" already means exactly the Admin dashboard + Hardening rows as originally scoped and the leftover Money/Trust items move to a later batch.
