...
**The architecture is the source of truth: [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md).** Read §0.1 for approved MVP decisions and the tax design. Build milestones in order; do not start a future milestone until the current one is approved.

## Current status

- **Milestone 1 (Foundations + Auth & Profiles): DONE & verified (2026-07-19).**
- Next up is **Milestone 2 (Requests, AI diagnosis, pricing)** — do **not** begin until the founder approves.
...
## Milestone 1 boundaries (deliberately deferred)

- **PostGIS/geo:** M1 stores lat/lng as `Decimal`. PostGIS `geography` + GIST indexes + radius dispatch arrive in **M3**.
- **Media uploads:** master certifications/portfolio store an `objectKey` string; the S3 presign/upload pipeline is a later milestone.
- **MyID / Didox / real SMS / payments / rules engine / trust tiers / warranty / admin dashboard:** all later milestones per the roadmap. Interfaces (SmsProvider, VerificationProvider, TaxProvider, PaymentProvider) exist so they slot in without rework.
- **CI/CD, ESLint config:** not set up in M1 (`next.config` skips lint during build).

## Key env vars

`DATABASE_URL`, `REDIS_URL`, `JWT_ACCESS_SECRET`, `JWT_ACCESS_TTL`, `JWT_REFRESH_TTL_DAYS`, `FIELD_ENCRYPTION_KEY`, `SMS_PROVIDER` (`mock`|`eskiz`) + `ESKIZ_*`, `TAX_PROVIDER` (`mock`|`soliq`) + `TAX_WITHHOLDING_RATE`, `WEB_ORIGIN`, `NEXT_PUBLIC_API_URL`. See `.env.example`.
